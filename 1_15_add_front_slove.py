...
class DWA:
    def __init__(self):
        # 设定机器人参量
        self.max_speed = 0.3  # 最大线速度
        self.max_yaw_rate = 0.6  # 最大角速度
        self.dt = 0.1  # 时间间隔
        ...
        self.angular_pid = PIDController(kp=10.0, ki=0.05, kd=0.5)  # 调整角速度的 PID 参数
        self.tag_sta = 0
        self.count = 0
        self.change_count = 0
        self.last_data = 0
    ...
    def detect_obstacles(self, ranges):
        # 激光雷达数据预处理，检测障碍物
        self.obstacles = []
        self.first_value = ranges[0]  # 获取 ranges 的第一个值56
        ...
        if index_90 < len(ranges) and index_270 < len(ranges) :
            self.value_at_90 = ranges[index_90]
            self.value_at_270 = ranges[index_270]
            # print("value_at_90:",self.value_at_90)
            # print("value_at_350:",self.value_at_350)
    ...
    def score_trajectory(self, trajectory):
        ...
        return -distance_to_target - 10 * obstacle_penalty 
    def run(self):
