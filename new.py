...
class DWA:
    def __init__(self):
        ...
        # 初始化障碍物列表
        self.obstacles = []
        self.linear_pid = PIDController(kp=10.0, ki=0.1, kd=0.5)  # 调整线速度的 PID 参数
        self.angular_pid = PIDController(kp=6.0, ki=0.05, kd=0.5)  # 调整角速度的 PID 参数
        # ROS 的发布器
        ...
        while not rospy.is_shutdown():
            ...
            if current_distance_to_target < 0.9:  # 如果距离小于0.5
                ...
                if best_trajectory is not None:
                    ...
                    # for v, omega in zip(v_list, omega_list):    
                    #     cmd = Twist()
                    #     cmd.linear.x = float(v)
                    #     cmd.angular.z = float(omega)
                    #     self.cmd_pub.publish(cmd)
                    #     rospy.sleep(0.1)  # 假设每个控制量持续 0.1 秒
                        v = best_v
                        omega = best_omega
                print("v",v)
                print("w",omega)
